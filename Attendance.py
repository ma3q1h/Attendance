...
import datetime, binascii, nfc, csv, time, sys, os, threading, ctypes
import pandas as pd
from tkinter import *
...
def show_app():
    global window_flag
    global root_flag
    if window_flag==False and root_flag==True:
        global root
        root.deiconify()
        window_flag=True
    else:
...
def close_window(root_window):   #https://qiita.com/mmura001/items/ca1906bc579f336fbc1c
    global window_flag
...
def run_app():
    global window_flag
    global root_flag
    if root_flag==False and window_flag==False:
...
image = Image.open('.\\ICON\\attendance.ico')
menu = Menu(MenuItem('Run', run_app), MenuItem('Show', show_app), MenuItem('Quit', quit_app))
icon = Icon(name='Attendance', icon=image, title='Attendance', menu=menu)
icon.run()